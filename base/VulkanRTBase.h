...
{																									\
	...
	vulkanFullRT->renderLoop();																	\
	delete(vulkanFullRT);																			\
	return 0;																						\
}
#define VULKAN_DEFERRED()																		\
VulkanDeferred *vulkanDeferred;																		\
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)						\
{																									\
	if (vulkanDeferred != NULL)																		\
	{																								\
		vulkanDeferred->handleMessages(hWnd, uMsg, wParam, lParam);									\
	}																								\
	return (DefWindowProc(hWnd, uMsg, wParam, lParam));												\
}																									\
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)									\
{																									\
	for (int32_t i = 0; i < __argc; i++) { VulkanDeferred::args.push_back(__argv[i]); };  			\
	vulkanDeferred = new VulkanDeferred();															\
	vulkanDeferred->initVulkan();																	\
	vulkanDeferred->setupWindow(hInstance, WndProc);													\
	vulkanDeferred->prepare();																		\
	vulkanDeferred->renderLoop();																	\
	delete(vulkanDeferred);																			\
	return 0;																						\
}
...
{																									\
	...
	vulkanHybrid->renderLoop();																	\
	delete(vulkanHybrid);																			\
	return 0;																						\
}
#define VULKAN_HYBRID_SHADOWMAP()																		\
VulkanHybridShadowmap *vulkanHybridShadowmap;																		\
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)						\
{																									\
	if (vulkanHybridShadowmap != NULL)																		\
	{																								\
		vulkanHybridShadowmap->handleMessages(hWnd, uMsg, wParam, lParam);									\
	}																								\
	return (DefWindowProc(hWnd, uMsg, wParam, lParam));												\
}																									\
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)									\
{																									\
	for (int32_t i = 0; i < __argc; i++) { VulkanHybridShadowmap::args.push_back(__argv[i]); };  			\
	vulkanHybridShadowmap = new VulkanHybridShadowmap();															\
	vulkanHybridShadowmap->initVulkan();																	\
	vulkanHybridShadowmap->setupWindow(hInstance, WndProc);													\
	vulkanHybridShadowmap->prepare();																		\
	vulkanHybridShadowmap->renderLoop();																	\
	delete(vulkanHybridShadowmap);																			\
	return 0;																						\
}
#elif defined(VK_USE_PLATFORM_ANDROID_KHR)
// Android entry point
...
{																									\
	vulkanHybridShadowmap = new VulkanHybridShadowmap();															\
	state->userData = vulkanHybridShadowmap;																\
	state->onAppCmd = VulkanRTBase::handleAppCommand;												\
	state->onInputEvent = VulkanRTBase::handleAppInput;											\
	androidApp = state;																				\
	vks::android::getDeviceConfig();																\

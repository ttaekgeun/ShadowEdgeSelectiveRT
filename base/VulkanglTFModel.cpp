...
void vkglTF::Scene::loadFromFile(std::string filename, vks::VulkanDevice* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	tinygltf::Model gltfModel;
	tinygltf::TinyGLTF gltfContext;
	if (fileLoadingFlags & FileLoadingFlags::DontLoadImages) {
	...
	std::vector<uint32_t> indexBuffer[MODEL_COUNT];
	std::vector<Vertex> vertexBuffer[MODEL_COUNT];

	if (fileLoaded) {

		for (int i = 0; i < MODEL_COUNT; i++)
		{
			SG_BLASBuildInfo _blasBuildInfo{};
			SG_BLAS* _blas = new SG_BLAS{};
			_blasBuildInfo.blas = _blas;
			_blasBuildInfo.blas->blasType = BLAS_TYPE_TRIANGLE_MESH;
			blasBuildInfo.push_back(_blasBuildInfo);
		}
	...
	if ((fileLoadingFlags & FileLoadingFlags::PreTransformVertices) || (fileLoadingFlags & FileLoadingFlags::PreMultiplyVertexColors) || (fileLoadingFlags & FileLoadingFlags::FlipY)) {
		...
		for (Node* node : linearNodes) {
			if (node->mesh) {
				...
				for (Primitive* primitive : node->mesh->primitives) {
					for (uint32_t i = 0; i < primitive->vertexCount; i++) {
						Vertex& vertex = vertexBuffer[node->blasIndex][primitive->firstVertex + i];
						// Pre-transform vertex positions by node-hierarchy
						if (preTransform) {
							vertex.pos = glm::vec3(localMatrix * glm::vec4(vertex.pos, 1.0f));
							vertex.normal = glm::normalize(glm::mat3(localMatrix) * vertex.normal);
	...
	size_t vertexBufferSize[MODEL_COUNT];
	size_t indexBufferSize[MODEL_COUNT];

	for (int i = 0; i < MODEL_COUNT; i++)
	{
		vertexBufferSize[i] = vertexBuffer[i].size() * sizeof(Vertex);
		indexBufferSize[i] = indexBuffer[i].size() * sizeof(uint32_t);

		blasBuildInfo[i].blas->blasData.triangleMesh.indices.count = static_cast<uint32_t>(indexBuffer[i].size());
		blasBuildInfo[i].blas->blasData.triangleMesh.vertices.count = static_cast<uint32_t>(vertexBuffer[i].size());

		assert((vertexBufferSize[i] > 0) && (indexBufferSize[i] > 0));
	}

	struct StagingBuffer {
		VkBuffer buffer;
		VkDeviceMemory memory;
	} vertexStaging[MODEL_COUNT], indexStaging[MODEL_COUNT];
	...
	for (int i = 0; i < MODEL_COUNT; i++)
	{
		VK_CHECK_RESULT(device->createBuffer(
			...
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
			vertexBufferSize[i],
			&vertexStaging[i].buffer,
			&vertexStaging[i].memory,
			vertexBuffer[i].data()));
		// Index data
		VK_CHECK_RESULT(device->createBuffer(
			VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
			indexBufferSize[i],
			&indexStaging[i].buffer,
			&indexStaging[i].memory,
			indexBuffer[i].data()));
	}
	...
	for (int i = 0; i < MODEL_COUNT; i++)
	{
		...
		VK_CHECK_RESULT(device->createBuffer(
			...
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
			vertexBufferSize[i],
			&blasBuildInfo[i].blas->blasData.triangleMesh.vertices.buffer,
			&blasBuildInfo[i].blas->blasData.triangleMesh.vertices.memory));
		...
		VK_CHECK_RESULT(device->createBuffer(
			...
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
			indexBufferSize[i],
			&blasBuildInfo[i].blas->blasData.triangleMesh.indices.buffer,
			&blasBuildInfo[i].blas->blasData.triangleMesh.indices.memory));
	}
	...
	for (int i = 0; i < MODEL_COUNT; i++)
	{
		copyRegion.size = vertexBufferSize[i];
		vkCmdCopyBuffer(copyCmd, vertexStaging[i].buffer, blasBuildInfo[i].blas->blasData.triangleMesh.vertices.buffer, 1, &copyRegion);
		copyRegion.size = indexBufferSize[i];
		vkCmdCopyBuffer(copyCmd, indexStaging[i].buffer, blasBuildInfo[i].blas->blasData.triangleMesh.indices.buffer, 1, &copyRegion);
	}
	...
	for (int i = 0; i < MODEL_COUNT; i++)
	{
		vkDestroyBuffer(device->logicalDevice, vertexStaging[i].buffer, nullptr);
		vkFreeMemory(device->logicalDevice, vertexStaging[i].memory, nullptr);
		vkDestroyBuffer(device->logicalDevice, indexStaging[i].buffer, nullptr);
		vkFreeMemory(device->logicalDevice, indexStaging[i].memory, nullptr);
	}
	...
	uint32_t instanceShaderBindingTableRecordOffset[MODEL_COUNT] =
	{
		SHADER_INDEX_BASIC,
		SHADER_INDEX_CUTOUT,
		SHADER_INDEX_REFLECTION_TRIANGLE
	};
	...
	//for (int i = 0; i < 2; i++)
	{
		...
		instance->blas = blasBuildInfo[i].blas;
...
void vkglTF::Scene::loadNode(vkglTF::Node* parent, const tinygltf::Node& node, uint32_t nodeIndex, const tinygltf::Model& model, std::vector<uint32_t>(&indexBuffer)[MODEL_COUNT], std::vector<Vertex>(&vertexBuffer)[MODEL_COUNT], float globalscale)
{
	...
	if (node.mesh > -1) {
		...
		{
			if (node.name == "CutOutObject")
				newNode->blasIndex = OBJECT_INDEX_CUTOUT;			// raytracing basic + any hit
			else if (node.name == "Statue")
				newNode->blasIndex = OBJECT_INDEX_REFLECTION;		// raytracing reflection
			else
				newNode->blasIndex = OBJECT_INDEX_BASIC;			// raytracing basic
		}
		...
		for (size_t j = 0; j < mesh.primitives.size(); j++) {
			...
			blasBuildInfo[newNode->blasIndex].geometryCount++;
			uint32_t indexStart = static_cast<uint32_t>(indexBuffer[newNode->blasIndex].size());
			uint32_t vertexStart = static_cast<uint32_t>(vertexBuffer[newNode->blasIndex].size());
			uint32_t indexCount = 0;
			uint32_t vertexCount = 0;
			glm::vec3 posMin{};
			glm::vec3 posMax{};
			...
			// Vertices
			{
				...
				for (size_t v = 0; v < posAccessor.count; v++) {
					...
					vert.joint0 = hasSkin ? glm::vec4(glm::make_vec4(&bufferJoints[v * 4])) : glm::vec4(0.0f);
					vert.weight0 = hasSkin ? glm::make_vec4(&bufferWeights[v * 4]) : glm::vec4(0.0f);
					vertexBuffer[newNode->blasIndex].push_back(vert);
				}
			}
			// Indices
			{
				...
				case TINYGLTF_PARAMETER_TYPE_UNSIGNED_INT: {
					...
					memcpy(buf, &buffer.data[accessor.byteOffset + bufferView.byteOffset], accessor.count * sizeof(uint32_t));
					for (size_t index = 0; index < accessor.count; index++) {
						indexBuffer[newNode->blasIndex].push_back(buf[index] + vertexStart);
					}
					delete[] buf;
					break;
				}
...
int vkglTF::Scene::getInstanceCount()
{
	return instances.size();
}

int vkglTF::Scene::getBlasCount() {

	return blasBuildInfo.size();
}

void vkglTF::Scene::drawScene(VkCommandBuffer commandBuffer)
{
	for (auto _blasBuildInfo : blasBuildInfo)
	{
		if (_blasBuildInfo.blas->blasType == BLAS_TYPE_TRIANGLE_MESH)
		{
			const VkDeviceSize offsets[1] = { 0 };
			vkCmdBindVertexBuffers(commandBuffer, 0, 1, &_blasBuildInfo.blas->blasData.triangleMesh.vertices.buffer, offsets);
			vkCmdBindIndexBuffer(commandBuffer, _blasBuildInfo.blas->blasData.triangleMesh.indices.buffer, 0, VK_INDEX_TYPE_UINT32);
			vkCmdDrawIndexed(commandBuffer, _blasBuildInfo.blas->blasData.triangleMesh.indices.count, 1, 0, 0, 0);
		}
	}
}
...
vec3 ApplyAttenuation(vec3 before, float distance)
{
    float att = 1.0f / (attenuationFactor.k0 + attenuationFactor.k1 * distance + attenuationFactor.k2 * pow(distance, 2.0f));
    vec3 after = att * before;
    return after;
}
